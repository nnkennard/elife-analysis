...
def GetInfo():
    ...
    n_to_rate = input("How many review sentences do you want to rate?\n(Fewer means more breaks): ")
...
def main(n_reviews): 
 ...
 # first time means new file
    if first_time == True: 
        ...
        for _, review_df in sentence_df.groupby("review_id"):
            ...
            for _, sentence_dct in review_df.iterrows():
                ...
                print()
                print("-"*100)
                print(f"SENTENCE {n_sentences} OF {len(review_df)} SENTENCES IN REVIEW {n_reviews} of {n_to_rate} REVIEWS")
                print(f"M_ID: {mid}\tR_ID: {rid}\tS_ID: {sid}")
                print("-"*50)
                pp.pprint(f"{sentence_dct['text']}")
                print("-"*100)
                
                evaluative = input("\tThis sentence subjectively evaluates the manuscript (0=no, 1=yes): ")
    ...
    if first_time == False:
        ...
        n_reviews = 0
        sentence_df = sentence_df[~sentence_df['identifier'].isin(already_reviewed)][: n_to_rate]
