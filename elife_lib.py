...
def build_dep_tree(sentence):
    ...
    for token in sentence.tokens:
        ...
        my_id, head, deprel, pos, tok = tuple([token_dict[key] for key in "id head deprel xpos text".split()])
...
def extract_dep_paths(sentence, max_len=2):
    ...
    while True:
        ...
        for child in curr_node.children:
            if curr_node.my_id:
                features.append((curr_node.deprel,
                            curr_node.pos, child.deprel, child.pos))
            stack.append(child)
    return features
